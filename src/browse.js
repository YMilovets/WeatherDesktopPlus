...
function getTimeReset() {
  timeMessage.textContent =
    "Обновлено: " + (timeReset < 1 ? "меньше" : timeReset) + " мин. назад";
  timeReset += 1;
}
...
function resetInfo() {
  ...
  socket.on("send-weather-params", ({ data: source }) => {
    if (source !== null) {
      timeReset = 0;
      ...
      document.body.style.backgroundImage =
        "url('../assets/fullscreen/" + source.icon + ".jpg')";
      ...
      params[2].textContent =
        source.pressure !== null
          ? Math.round(source.pressure * config.mmhg) + " мм рт. ст."
          : "нет данных";
      params[3].textContent = Math.round(source.feelLikeTemp) + "°C";
      const { name, link } = config.services.find(
        ({ type }) => type === source.service
      );
