const { app, dialog, Menu, Tray, shell, Notification } = require("electron");
...
function main() {
  ...
  let tray = new Tray(
    path.join(__dirname, "assets/tray", config.default.trayStyle, "03d.png")
  );
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    {
      label: "Обновить данные по погоде",
      click() {
  ...
  socket.on("send-weather-params", ({ data, trayStyle }) => {
    if (isEmpty(data)) getTrayCanceled();
    else {
      getTrayAccepted(data, trayStyle);
    }
  });
  socket.on("send-error", (title, data) => {
    ...
    new Notification({ title, body: data }).show();
  });
  ...
  function getTrayCanceled() {
    ...
    tray.setImage(__dirname + "/assets/tray/default/01d.png");
  }
  function getTrayAccepted(src, trayStyle) {
    tray.setToolTip(src.city + " " + Math.round(src.temperature) + "°C");
    tray.setImage(
      path.join(__dirname, "/assets/tray", trayStyle, `${src.icon}.png`)
    );
  }
}
